...
// ParseWebhook parses a webhook body for gitea
func (c *Client) ParseWebhook(header http.Header, jsonString []byte) (*git.Webhook, error) {
	...
	eventType := git.EventType(header.Get("X-Gitea-Event"))
	switch eventType {
...
func (c *Client) ListWebhook() ([]git.WebhookEntry, error) {
	var apiURL = c.IntegrationConfig.Spec.Git.GetAPIUrl() + "/api/v1/repos/" + c.IntegrationConfig.Spec.Git.Repository + "/hooks"
...
func (c *Client) RegisterWebhook(url string) error {
	...
	registrationBody.Events = []string{"push", "pull_request", "issue_comment", "pull_request_review", "pull_request_review_comment", "commit_comment"}
	registrationBody.Active = true
	registrationBody.Type = "gitea"
...
func (c *Client) DeleteWebhook(id int) error {
	var apiURL = c.IntegrationConfig.Spec.Git.GetAPIUrl() + "/api/v1/repos/" + c.IntegrationConfig.Spec.Git.Repository + "/hooks/" + strconv.Itoa(id)
...
func (c *Client) ListCommitStatuses(ref string) ([]git.CommitStatus, error) {
	apiURL := c.IntegrationConfig.Spec.Git.GetAPIUrl() + "/api/v1/repos/" + c.IntegrationConfig.Spec.Git.Repository + "/commits/" + ref + "/statuses"
...
func (c *Client) SetCommitStatus(sha string, status git.CommitStatus) error {
	...
	apiURL := c.IntegrationConfig.Spec.Git.GetAPIUrl() + "/api/v1/repos/" + c.IntegrationConfig.Spec.Git.Repository + "/statuses/" + sha
...
func (c *Client) GetUserInfo(userName string) (*git.User, error) {
	// userName is string!
	apiURL := fmt.Sprintf("%s/api/v1/user/%s", c.IntegrationConfig.Spec.Git.GetAPIUrl(), userName)
...
func (c *Client) CanUserWriteToRepo(user git.User) (bool, error) {
	// userName is string!
	apiURL := fmt.Sprintf("%s/api/v1/repos/%s/collaborators/%s/permission", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, user.Name)
...
func (c *Client) RegisterComment(issueType git.IssueType, issueNo int, sha, body string) error {
	...
	if issueType == git.IssueTypePullRequest {
		apiUrl = fmt.Sprintf("%s//api/v1/repos/%s/issues/%d/comments", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, issueNo)
	} else if issueType == git.IssueTypeCommit {
		apiUrl = fmt.Sprintf("%s//api/v1/repos/%s/commits/%s/comments", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, sha)
	}
...
func (c *Client) ListComments(issueNo int) ([]git.IssueComment, error) {
	...
	issueApiUrl := fmt.Sprintf("%s//api/v1/repos/%s/issues/%d/comments", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, issueNo)
	prCommentApiUrl := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d/comments", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, issueNo)
	prReviewApiUrl := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d/reviews", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, issueNo)
...
func (c *Client) ListPullRequests(onlyOpen bool) ([]git.PullRequest, error) {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/pulls", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository)
	if !onlyOpen {
		apiURL += "?state=all"
	}
...
func (c *Client) GetPullRequest(id int) (*git.PullRequest, error) {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id)
...
func (c *Client) MergePullRequest(id int, sha string, method git.MergeMethod, message string) error {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d/merge", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id)
...
func (c *Client) GetPullRequestDiff(id int) (*git.Diff, error) {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d/files", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id)
...
func (c *Client) ListPullRequestCommits(id int) ([]git.Commit, error) {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/pulls/%d/commits", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id)
...
func (c *Client) SetLabel(_ git.IssueType, id int, label string) error {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/issues/%d/labels", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id)
...
func (c *Client) DeleteLabel(_ git.IssueType, id int, label string) error {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/issues/%d/labels/%s", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, id, label)
...
func (c *Client) GetBranch(branch string) (*git.Branch, error) {
	apiURL := fmt.Sprintf("%s//api/v1/repos/%s/branches/%s", c.IntegrationConfig.Spec.Git.GetAPIUrl(), c.IntegrationConfig.Spec.Git.Repository, branch)
